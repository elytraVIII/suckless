...
#include "../util.h"
	...
	vol_perc(const char *card)
	{
		...
		if ((fp = popen("wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null", "r"))) {
			if (fscanf(fp, "Volume: %lf %15s", &vol, mute) >= 1) {
				pclose(fp);
				if (strcmp(mute, "[MUTED]") == 0) {
					return bprintf("0");
				}
				return bprintf("%d", (int)(vol * 100 + 0.5));
			}
			pclose(fp);
		}
